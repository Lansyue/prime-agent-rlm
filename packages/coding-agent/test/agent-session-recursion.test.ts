...
describe("AgentSession rlm recursion", () => {
	...
	it("keeps RLM descendants in memory when the root session is non-persisted", async () => {
		...
		expect(child.sessionManager.allowsPersistence()).toBe(false);
		...
		expect(grandchild.sessionManager.allowsPersistence()).toBe(false);
	...
	it("runs a child session under a sub directory and returns an RLM-shaped result", async () => {
		...
		expect(conversationMessages(child)[0]).toMatchObject({
			role: "custom",
	...
	it("surfaces a child turn that exhausts empty-response retries as a failure to the parent", async () => {
