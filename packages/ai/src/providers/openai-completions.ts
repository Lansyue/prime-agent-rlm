...
export const streamOpenAICompletions: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		try {
			...
			// The session's "do not resend a permanent failure" gate reads this diagnostic
			// only; without it a permanent 400 is retried like a transient outage.
...
function buildParams(
	...
	if (context.tools && context.tools.length > 0) {
		...
		if (compat.toolStream !== undefined) {
			(params as any).tool_stream = compat.toolStream;
		} else if (compat.zaiToolStream) {
